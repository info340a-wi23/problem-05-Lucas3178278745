...
let UWlosts = huskyGames2016.filter(huskiesLost);
console.log(UWlosts);

//Log out an array of opponents that UW lost to. Hint: Use the `.map()` method 
//to extract the opponent names!
let WhoBeatsUW = UWlosts.map(extractOpponent);
console.log(WhoBeatsUW);
...
UWlosts.forEach(function(game) {
  let opponent = extractOpponent(game);
  let homeScore, opponentScore;
  if (game.home === 'UW') {
  ...
  console.log(`${opponent} at UW, ${opponentScore} to ${homeScore}`);
});
...
let fumbles = huskyGames2016.filter(function(game) {
...
function mostYardsPassing(game1, game2) {
  if (!Object.prototype.hasOwnProperty.call(game1, 'passing_yards')) {
    return game2;
  } else {
...
let fumbleGames = function(game) { return game.fumbles > 0 };
let fumbledAndLostFilter = makeCombinedFilter(huskiesLost, fumbleGames);
...
let lostAndFumbledGames = huskyGames2016.filter(fumbledAndLostFilter);
console.log(lostAndFumbledGames);
